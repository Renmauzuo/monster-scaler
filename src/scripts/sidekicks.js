...
function buildSelectedSidekick() {
    ...
    for (let i = 0; i < earnedFeatures.length; i++) {
        ...
        //We need to add the trait before making any modifications to it below
        if (feature.trait) {
        ...
        if (feature.options) {
            for (let j = 0; j < feature.options.length; j++) {
                ...
                //Invoke the option callback if necessary
                if (option.result) {
                ...
                if (option.limit) {
                    let limit;
                    if (typeof(option.limit) === 'function') {
                        limit = option.limit(sidekickStats);
                    } else {
                        limit = option.limit;
                    }
                    $select.data('limit', limit);
                }
            
                //Reapply the value if there is one so it's not lost every refresh
                if (value) {
                    $select.val(value);
                } else {
        ...
        if (feature.bonusAction) {
            //Make sure bonus action list exists
            sidekickStats.bonusActions = sidekickStats.bonusActions || {};

            //Copy the relevant action
            sidekickStats.bonusActions[feature.bonusAction] = actions[feature.bonusAction];
        }
    }
    ...
    for (let i = 0; i < selectedClass.features.length; i++) {
        ...
        if (feature.options) {
            for (let j = 0; j < feature.options.length; j++) {
                ...
                let labelText = featureOption.label;
                ...
                if (featureOption.limit) {
                    ...
                    let limit;
                    if (typeof(featureOption.limit) === 'function') {
                        //Make sure the statblock is built before using it (can sometimes happen on initial load)
                        if (!sidekickStats) {
                            buildSelectedSidekick();
                        }
                        limit = featureOption.limit(sidekickStats);
                    } else {
                        limit = featureOption.limit;
                    }
                    $select.attr('data-limit', limit);
                    labelText += " (Select " + limit + ")";
                }

                $('<label for='+featureOption.id+'>'+labelText+'</label>').appendTo($wrapper);

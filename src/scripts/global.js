...
function renderStatblock(sourceStats) {
    ...
    $('#monster-name').html((sourceStats.wildShape && sourceStats.defaultName && sourceStats.defaultName !== sourceStats.name) ? (sourceStats.name + ' (' + sourceStats.defaultName + ')') : sourceStats.name);
    ...
    if (sourceStats.bonusHP) {
        bonusHP += sourceStats.bonusHP;
    }
    ...
    if (sourceStats.saves || sourceStats.saveBonus) {
        //Show saves if the creature is proficient in them
        //All saves are shown if there is a flat save bonus (such as from a protection item)
        $('#saving-throws').show();
        let saveString = "";

        for (let ability in abilities) {
            let saveModifier = 0;
            if (sourceStats.saves && sourceStats.saves.includes(ability)) {
                saveModifier += sourceStats.proficiency;
            }
            if (sourceStats.saveBonus) {
                saveModifier += sourceStats.saveBonus;
            }
            //Only show a save if bonus NOT count ability score (ie proficiency and/or any magic bonuses) is greater than 0
            //Otherwise, if the save bonus is simply the flat ability bonus it is ommitted
            if (saveModifier > 0) {
                if (saveString.length) {
                    saveString += ', ';
                }
                saveModifier += sourceStats.abilityModifiers[ability];
                let modifierString = (saveModifier >= 0 ? '+' : '') + saveModifier;
                saveString+= abilities[ability].name + ' ' + modifierString;
            }
        }

...
function calculateSelectedMonster() {
  ...
  derivedStats.proficiency = averageStats[targetCR].proficiency; //Once we have our locked stats, go through the rest of the states to interpolate or extrapolate based on existing values.
  ...
  if (derivedStats.attacks) {
    for (let attack in derivedStats.attacks) {
      ...
      let attackBonus = derivedStats.proficiency + abilityModifier; //Monster stat blocks never have negative to hit, so we set 0 as the minimum
      ...
      let maxDamage = currentAttack.damageDice * currentAttack.damageDieSize + abilityModifier; //If the attack can't do more than one damage total omit the roll and just show 1 flat damage

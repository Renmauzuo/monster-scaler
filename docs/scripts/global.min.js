...
function scaleMonster(monsterID, targetCR, options = {}) {
  ...
  if (selectedMonster.type === typeHumanoid) {
    if (selectedMonster.race === raceAny) {
      ...
      derivedStats.type = selectedMonster.type + ' (' + currentRace.name + ')'; //If not any race apply the mods for the chosen race
  ...
  derivedStats.description = derivedStats.unique ? derivedStats.name : 'the ' + derivedStats.slug; //Traits require a different approach from some other stats as we take a base from the trait library, but potentially apply modifiers to it based on creature stats.

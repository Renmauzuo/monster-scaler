...
function scaleMonster(monsterID, targetCR, options = {}) {
  ...
  if (type === typeHumanoid) {
    if (selectedMonster.race === raceAny) {
      ...
      derivedStats.type = type + ' (' + currentRace.name + ')'; //If not any race apply the mods for the chosen race

      if (currentRace !== races[0]) {
        derivedStats = mergeObjects(derivedStats, currentRace.stats); //If the NPC gets languages for their race deduct them from any "bonus languages"
  ...
  derivedStats.proficiency = averageStats[targetCR].proficiency; //Store some strings in derived stats so they are available outside this scope

  derivedStats.alignment = selectedMonster.alignment; //Once we have our locked stats, go through the rest of the states to interpolate or extrapolate based on existing values.
  ...
  if (derivedStats.bonusArmor == undefined) {
    ...
    */
    let acBenchmarks = findBenchmarksForStat(["bonusArmor", "dex"], targetCR, sourceStats); //Creature may not have bonus armor at all, in which case we skip this step
    ...
    if (acBenchmarks) {
      ...
      let targetAC = extrapolateFromBenchmark('ac', targetCR, acBenchmarks, false); //The max check shouldn't really be necessary, but we don't want to risk a creature with abnormally high dex resulting in a negative bonus armor rating
  ...
  if (!derivedStats.hitDice) {
    ...
    let hpStats = ["con", "hitDice"]; //Don't search for size for humanoids as their size is locked (and will cause an error)
  ...
  for (let attack in derivedStats.attacks) {
    let currentAttack = derivedStats.attacks[attack]; //Ensure we are using appropriate stats for the target CR, if they are present
    ...
    if (!currentAttack.damageDice) {
      ...
      let targetDamage = estimatedDamage - (currentAttack.finesse ? Math.max(derivedStats.abilityModifiers.str, derivedStats.abilityModifiers.dex) : derivedStats.abilityModifiers.str); //console.log(targetDamage);
  ...
  if (selectedMonster.type === typeHumanoid && selectedMonster.race === raceAny && currentRace !== races[0]) {
    for (let stat in currentRace.bonusStats) {
      derivedStats[stat] += currentRace.bonusStats[stat]; //This does mean we need to recalculate the modifier
  ...
  derivedStats.fly = findNearestLowerBenchmark('fly', targetCR, sourceStats); //Determine what senses the creature should have
...
function renderStatblock(sourceStats) {
  ...
  if (sourceStats.bonusArmor) {
    acTotal += sourceStats.bonusArmor; //Only add an armor description if we didn't already get one from worn armor
  ...
  $('#armor-class span').html(armorString); //Dwarves make HP more complicated

  let bonusHP = sourceStats.abilityModifiers.con * sourceStats.hitDice; //Dwarven toughness is only added here so it doesn't change the hit dice of dwarf NPCs
  ...
  if (sourceStats.attacks) {
    for (let attack in sourceStats.attacks) {
      ...
      let maxDamage = currentAttack.damageDice * currentAttack.damageDieSize + currentAttack.damageBonus; //If the attack can't do more than one damage total omit the roll and just show 1 flat damage
...
function damageString(damageDice, damageDieSize, damageBonus = 0) {
  let maxDamage = damageDice * damageDieSize + damageBonus; //If the attack can't do more than one damage total omit the roll and just show 1 flat damage
...
function findDamageDice(targetDamage, preferredDieSize) {
  ...
  preferredDieSize = Math.max(preferredDieSize, 4); //TODO: Consider capping damage dice for specific attacks and split high damage attacks into multiattacks
  ...
  let dieAverages = [6.5, 5.5, 4.5, 3.5, 2.5]; //If the preferred die is a larger one make sure that a single die is not too much.
...
function exportFightClub(statblock) {
  ...
  fightClubXML += xmlNode('cha', statblock.cha); //TODO: Saves
  ...
  for (let traitName in statblock.traits) {
    let traitXML = xmlNode('name', statblock.traits[traitName].name); //We need to remove HTML children and replace them with appropriate XML entities for FightClub
...
function serializeForm($form) {
  ...
  $form.find('select:visible').each(function () {
    let value = $(this).val(); //Some selects default to 0 so we can comfortably skip it
...
function deserializeQuery() {
  if (location.search.length) {
    let params = new URLSearchParams(location.search); //Skip variant at first because it must come after monster

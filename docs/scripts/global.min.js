...
$(function () {
  ...
  if (location.search.length) {
    ...
    let paramsCR = params.get('cr'); //Ensure it's a valid monster before selecting
...
function calculateSelectedMonster() {
  ...
  $('#direct-link').attr('href', directLink); //Start with locked stats and presets for this CR, if any
  ...
  if (!derivedStats.naturalArmor) {
     ...
     */
    let acBenchmarks = findBenchmarksForStat(["naturalArmor", "dex"], targetCR, selectedMonster); //Creature may not have natural armor at all, in which case we skip this step
    ...
    if (acBenchmarks) {
      ...
      let targetAC = extrapolateFromBenchmark('ac', targetCR, acBenchmarks, false); //The max check shouldn't really be necessary, but we don't want to risk a creature with abnormally high dex resulting in a negative natural armor rating
  ...
  derivedStats.attacks = {}; //TODO: Account for creatures that may gain attacks as they go up in CR
  ...
  for (let attack in selectedMonster.attacks) {
    ...
    if (!derivedStats.attacks[attack].damageDice) {
      ...
      let targetDamage = estimatedDamage - (derivedStats.attacks[attack].finesse ? Math.max(derivedStats.abilityModifiers.str, derivedStats.abilityModifiers.dex) : derivedStats.abilityModifiers.str); //console.log(targetDamage);
  ...
  $('#monster-type').html(sizes[derivedStats.size].name + ' ' + selectedMonster.type + ', ' + selectedMonster.alignment); //TODO: Pick the appropriate AC formula if a creature has options (ie, natural armor vs worn armor)

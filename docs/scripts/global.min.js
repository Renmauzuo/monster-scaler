...
$(function () {
  ...
  if (location.search.length) {
    ...
    let paramsName = params.get('name'); //Ensure it's a valid monster before selecting
    ...
    setupVariantSelect(false); //Select the variant if it has one
...
function calculateSelectedMonster() {
  ...
  $('#direct-link').attr('href', directLink); //Need to combine variant stats with base stats, if applicable
  ...
  if (selectedMonster.type === typeHumanoid) {
    if (selectedMonster.race === raceAny) {
      ...
      $('#race-wrapper').show(); //If not any race apply the mods for the chosen race
  ...
  let bonusHP = derivedStats.abilityModifiers.con; //Dwarven toughness is only added here so it doesn't change the hit dice of dwarf NPCs
  ...
  if (derivedStats.attacks) {
    for (let attack in derivedStats.attacks) {
      ...
      let abilityModifier = currentAttack.finesse ? Math.max(derivedStats.abilityModifiers.str, derivedStats.abilityModifiers.dex) : derivedStats.abilityModifiers.str; //Save some things for Fight Club export
...
function exportFightClub() {
  ...
  fightClubXML += xmlNode('cha', derivedStats.cha); //TODO: Saves

...
function calculateSelectedMonster() {
  ...
  if (selectedMonster.type === typeHumanoid) {
    if (selectedMonster.race === raceAny) {
      ...
      monsterStats.type = selectedMonster.type + ' (' + currentRace.name + ')'; //If not any race apply the mods for the chosen race

      if (currentRace !== races[0]) {
        monsterStats = mergeObjects(monsterStats, currentRace.stats); //If the NPC gets languages for their race deduct them from any "bonus languages"
  ...
  monsterStats.description = monsterStats.unique ? monsterStats.name : 'the ' + monsterStats.slug; //Traits require a different approach from some other stats as we take a base from the trait library, but potentially apply modifiers to it based on creature stats.
  ...
  for (let attack in monsterStats.attacks) {
    let currentAttack = monsterStats.attacks[attack]; //Ensure we are using appropriate stats for the target CR, if they are present
    ...
    if (!currentAttack.damageDice) {
      ...
      let targetDamage = estimatedDamage - (currentAttack.finesse ? Math.max(monsterStats.abilityModifiers.str, monsterStats.abilityModifiers.dex) : monsterStats.abilityModifiers.str); //console.log(targetDamage);
  ...
  if (selectedMonster.type === typeHumanoid && selectedMonster.race === raceAny && currentRace !== races[0]) {
    for (let stat in currentRace.bonusStats) {
      ...
      monsterStats[stat] += currentRace.bonusStats[stat]; //This does mean we need to recalculate the modifier
  ...
  monsterStats.fly = findNearestLowerBenchmark('fly', targetCR, sourceStats); //Determine what senses the creature should have

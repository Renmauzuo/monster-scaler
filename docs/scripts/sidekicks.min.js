    ...
    if (feature.trait) {
      ...
      if (feature.options) {
        for (let j = 0; j < feature.options.length; j++) {
          ...
          option.result(sidekickStats, value); //Refresh the option list in case any other changes affect it

          let list = typeof option.list === 'function' ? option.list(sidekickStats) : option.list;
          $select.empty();
          populateSelect(list, $select); //Reapply the value if there is oneso it's not lost every refresh

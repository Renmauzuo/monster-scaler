...
6. WHEN `statblock.legendaryResistances` is set to a positive number, THE Extractor SHALL classify Legendary Resistance as a Limited_Use_Ability with `key: 'legendaryResistance'`, `name: 'Legendary Resistance'`, `max: statblock.legendaryResistances`, `remaining: statblock.legendaryResistances`, and `recharge: 'long'` — displayed with checkboxes like all other Limited_Use_Abilities. The `legendaryResistance` trait SHALL be skipped during the Rest_Ability classification pass (AC 3 and AC 4) so it is not double-counted.
7. WHEN a trait or action description contains a pattern matching `(\d+)/day`, THE Extractor SHALL classify it as a Limited_Use_Ability and record the maximum use count. Note: spell-like abilities such as a dryad's "3/day" spells are correctly handled by this pattern on the trait description — no special-casing is needed.
8. WHEN a trait or action description contains a pattern matching `(\d+)/encounter`, THE Extractor SHALL classify it as a Limited_Use_Ability and record the maximum use count.
8. WHEN a trait or action description contains a pattern matching `(\d+)/encounter`, THE Extractor SHALL classify it as a Limited_Use_Ability and record the maximum use count.
9. THE Extractor SHALL produce a ResourceState containing: current HP (initialized to the statblock's maximum HP), one entry per Recharge_Ability (available/spent), one entry per Rest_Ability (available/spent), one entry per spell level with remaining slot count, one entry per Limited_Use_Ability with remaining use count, one boolean entry per non-immune non-exhaustion condition (initialized to inactive), and an exhaustion level (initialized to 0).
10. THE `@toolkit5e/base` package SHALL expose `recharge?: 'short' | 'long'` as a first-class optional field on the `Trait` interface, and relevant traits in `data.ts` and monster templates in `monsters.ts` SHALL be annotated with the appropriate `recharge` value so that any consumer of the package can access this structured data without parsing description text.
